...
- **Builtins** — in-process, no network, always available when
  ...
  - `list_llm_providers` / `list_mcp_hosts` — enumerate the
    server-level registries backing this pod. Each entry is tagged
    `in scope` when the pod's `[allow]` table admits it (usable now)
    or `not in scope` when it's registered on the server but would
    require editing `pod.toml` (and therefore `pod_modify`) to bring
    into scope. Use these to discover what model backends and shared
    MCP hosts exist before picking one for a `dispatch_thread` or
