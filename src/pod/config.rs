...
/// One entry from `[[auth.daemons]]`. `name` is the daemon's
/// identity; the daemon presents `token` as
/// `Authorization: Bearer <token>` on the `/v1/host_env_link`
/// WebSocket upgrade.
#[derive(Deserialize, Debug, Clone, PartialEq, Eq)]
...
impl Config {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        let mut daemon_names: std::collections::HashSet<&str> = std::collections::HashSet::new();
        for d in &self.auth.daemons {
            let name = d.name.trim();
            if name.is_empty() {
                return Err(anyhow!("config: auth.daemons entry has empty name"));
            }
            if d.token.trim().is_empty() {
                return Err(anyhow!("config: auth.daemons `{name}` token is empty"));
            }
            if !daemon_names.insert(name) {
                return Err(anyhow!("config: auth.daemons has duplicate name `{name}`"));
            }
        }
        Ok(())
    }
...
"#;
    ...
    fn cfg_with_daemons(daemons: Vec<AuthDaemon>) -> Config {
        let text = r#"
[backends.cloud]
kind = "anthropic"
auth = { mode = "api_key", value = "sk-test" }
"#;
        ...
        cfg.auth.daemons = daemons;
        cfg
    }

    #[test]
    fn validate_rejects_duplicate_daemon_names() {
        let cfg = cfg_with_daemons(vec![daemon("beta", "t1"), daemon("beta", "t2")]);
        let err = cfg.validate().unwrap_err();
        assert!(format!("{err}").contains("duplicate name `beta`"), "{err}");
    }

    #[test]
    fn validate_rejects_empty_daemon_token() {
        let cfg = cfg_with_daemons(vec![daemon("beta", "   ")]);
        let err = cfg.validate().unwrap_err();
        assert!(format!("{err}").contains("token is empty"), "{err}");
    }

    #[test]
    fn parses_auth_daemons_table() {
        let text = r#"
[backends.cloud]
kind = "anthropic"
auth = { mode = "api_key", value = "sk-test" }
