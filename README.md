<p align="center">
  <img src="crates/whisper-agent-damascene-ui/assets/favicon-192.png" alt="whisper-agent" width="128" height="128">
</p>
...
- A web UI (damascene+wgpu+wasm, served by the agent itself) for driving long-lived conversations from any browser.
...
│    compatible local)                                        │
│  • serves the webui (damascene+wgpu+wasm) and a CBOR/WebSocket  │
...
**Server-side** (one container in deployment): `whisper-agent`, `whisper-agent-damascene-ui` (wasm bundle served by the agent), `whisper-agent-mcp-fetch`, `whisper-agent-mcp-search`.
...
| `whisper-agent-damascene-ui` | Chat UI (damascene + wgpu + wasm), built with `wasm-pack`. Same crate produces the rlib that powers `whisper-agent-desktop-damascene`. |
...
| `whisper-agent-desktop-damascene` | Native desktop client (winit + wgpu) sharing UI code with `whisper-agent-damascene-ui`. |
