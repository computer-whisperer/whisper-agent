...
function on_event(state, event, config)
  ...
  if k == "agent_completed" then
    if state.title_thread and event.thread_id == state.title_thread then
      ...
      -- First round closed and the title resolved: the triggered
      -- unit of work is done. For a behavior-spawned weave this
      -- records the run (step 11 slice 2); elsewhere it journals
      -- and moves nothing.
  ...
  if k == "thread_failed" then
    if state.title_thread and event.thread_id == state.title_thread then
      ...
      -- don't retry (title_requested stays set). The round itself
      -- closed, so the triggered run still completed — titling is
      -- cosmetic.
      state.title_thread = nil
