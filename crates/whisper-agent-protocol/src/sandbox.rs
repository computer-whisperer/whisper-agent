...
/// connect, and `mcp_token` is the per-sandbox bearer the MCP host requires
/// on every request. The token is generated by the daemon, scoped to this
/// one sandbox's lifetime, and never reused.
#[derive(Serialize, Deserialize, Debug, Clone)]
...
mod tests {
    ...
    #[test]
    fn provision_response_round_trips_including_token() {
        let resp = ProvisionResponse {
            session_id: "thread-xyz".into(),
            mcp_url: "http://127.0.0.1:9820/mcp".into(),
            mcp_token: "deadbeefcafef00d".into(),
        };
        let json = serde_json::to_string(&resp).unwrap();
        let back: ProvisionResponse = serde_json::from_str(&json).unwrap();
        assert_eq!(back.session_id, resp.session_id);
        assert_eq!(back.mcp_url, resp.mcp_url);
        assert_eq!(back.mcp_token, resp.mcp_token);
    }
