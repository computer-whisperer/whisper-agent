...
//! scheduler asks a v2 daemon to provision).
//!
//! Each `[[allow.host_env]]` entry on a pod carries a `HostEnvSpec`
//! plus the `provider` name that will be asked to provision it. The
//! spec is the wire-level input the daemon receives over the v2
//! host-env link; it does not itself know which daemon will fulfill it
//! (that's resolved server-side against `[[auth.daemons]]`).
//!
...
/// Execution environment a v2 daemon can provision. A
