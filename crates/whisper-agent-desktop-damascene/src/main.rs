//! whisper-agent-desktop-damascene — native window around the damascene chat UI.
//!
//! WebSocket transport is tokio + tungstenite + CBOR over `/ws`;
//! render stack is `damascene-winit-wgpu`.
//!
//! Two phases:
...
//!
//! Threading:
//!   - main thread: winit event loop + damascene `Runner` paint
...
//!
//! Wakeup caveat: damascene-winit-wgpu doesn't currently expose a
...
//! ideally damascene grows a wakeup channel and we drop the cadence.
...
#[command(
    version,
    about = "whisper-agent-desktop-damascene: native window around the whisper-agent damascene ui"
)]
struct Args {
...
fn main() -> Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| EnvFilter::new("info,whisper_agent_desktop_damascene=info")),
        )
        .init();
    ...
    let host_config =
        damascene_winit_wgpu::HostConfig::default().with_redraw_interval(Duration::from_millis(33));
    damascene_winit_wgpu::run_with_config("whisper-agent", viewport, root, host_config)
        .map_err(|e| anyhow!("damascene runner: {e}"))?;
    Ok(())
}
...
/// Top-level damascene [`App`] that switches between the login form and
