//! Dump damascene bundle artifacts (SVG, tree dump, draw_ops, lint, manifest)
//! for each known scene of [`ChatApp`]. CPU-only — no window, no GPU.
//!
//! Usage: `cargo run -p whisper-agent-damascene-ui --example dump_bundles`
//!
//! Output: `crates/whisper-agent-damascene-ui/out/<scene>.{svg,tree.txt,
...
enum Scene {
    ...
    /// decode path (PNG bytes → `damascene_core::Image`), the height
