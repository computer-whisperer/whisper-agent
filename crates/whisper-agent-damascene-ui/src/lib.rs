//! whisper-agent-damascene-ui — chat UI built on damascene.
//!
...
//!     `/pkg/whisper_agent_damascene_ui.js`. Owns the wgpu+winit host
//!     shell + WebSocket transport for the browser.
//!   - Native: `whisper-agent-desktop-damascene` consumes this crate as
//!     an rlib, wraps it in `damascene-winit-wgpu`, and runs its own
//!     tokio+tungstenite WS bridge.
//!
//! The crate owns two platform-agnostic damascene [`App`]s: [`ChatApp`]
