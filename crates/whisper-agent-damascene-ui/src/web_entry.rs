//! Wasm browser entry for the damascene chat UI.
//!
...
//! [`damascene_web`]; we just call [`damascene_web::start_with_config`] with
...
//!   continue through damascene-web's built-in path (which forwards to
//!   the focused Damascene text input).
//!
...
use damascene_web::{WebHandle, WebHostConfig, start_with_config};
...
/// Logical viewport supplied to `WebHostConfig`. damascene-web's
...
/// The HTML `id` of the canvas the bundled `index.html` ships. damascene-web
/// defaults to `damascene_canvas`; we override via `with_canvas_id` so the
/// existing host page keeps working.
const CANVAS_ID: &str = "the_canvas_id";

#[wasm_bindgen(start)]
pub fn start() {
    // Open the WebSocket eagerly so its connect handshake races damascene-web's
    ...
    // Hand the app to damascene-web. `start_with_config` schedules the
...
/// clipboard items only. damascene-web installs its own paste listener
...
/// and damascene-web routes the text as usual.
fn install_image_paste_handler(ingress: AttachmentIngress, handle: WebHandle) {
    ...
    let cb = Closure::wrap(Box::new(move |e: web_sys::ClipboardEvent| {
        ...
        if handled_image {
            // Eat the event so damascene-web's bubble-phase text listener
        ...
        // Text-only payloads fall through to damascene-web's listener.
