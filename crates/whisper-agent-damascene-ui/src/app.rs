//! [`ChatApp`] — the platform-agnostic damascene [`App`] for the
...
//!   through `damascene_markdown` with math enabled; reasoning and tool
...
use damascene_core::widgets::select::{SelectAction, classify_event as classify_select_event};
...
enum DisplayItem {
    ...
    /// `damascene_core::Image` so rebuilds don't re-decode the bytes.
    Image {
        role: ImageRole,
...
/// / `CreateThread` on submit) and a pre-decoded `damascene_core::Image`
...
struct StagedAttachment {
    ...
    thumbnail: Option<damascene_core::image::Image>,
...
/// after a push so the next frame drains the queue; on damascene there's
...
/// (we hold the `damascene_core::Image` and the original dimensions),
...
/// Decode an [`ImageSource::Bytes`] payload into an `damascene_core::Image`,
        ...
        ImageSource::Bytes { data, media_type } => match image::load_from_memory(data) {
            Ok(decoded) => {
                ...
                ImageRenderState::Decoded {
                    image: damascene_core::image::Image::from_rgba8(w, h, pixels),
                    width: w,
                    height: h,
                    bytes: data.clone(),
                    mime: *media_type,
...
pub struct ChatApp {
    ...
    /// `damascene_core::Image` is shared with the inline row so we don't
    ...
    /// widgets. Damascene's controlled widgets read it through
    ...
    /// produced by [`damascene_core::widgets::accordion::accordion_item_key`]).
...
impl App for ChatApp {
    ...
    fn on_event(&mut self, event: UiEvent, _cx: &EventCx) {
        ...
        // the set).
        let pod_tab_changed = damascene_core::widgets::tabs::apply_event(
            &mut self.pod_tab,
            &event,
            POD_TABS_KEY,
            |raw| {
        ...
        // compose box, fold the event through damascene's controlled-
        ...
        if event.target_key() == Some(COMPOSE_KEY) {
            ...
            // attachment instead of inserting text. Mirrors damascene's
    ...
    fn theme(&self) -> Theme {
        // Damascene's slate + blue dark palette (Radix Colors). Card and
...
/// damascene's `toggle_group_multi` is a non-wrapping row and the pod
...
/// pod editor sheet is 360 px) where damascene's non-wrapping
...
/// `damascene_core::widgets::select::classify_event`, which would otherwise
...
impl ChatApp {
    ...
    fn stage_raw_pick(&mut self, pick: RawPick) {
        ...
        let thumbnail = image::load_from_memory(&pick.bytes).ok().map(|decoded| {
            let rgba = decoded.to_rgba8();
            let (w, h) = rgba.dimensions();
            damascene_core::image::Image::from_rgba8(w, h, rgba.into_raw())
        });
    ...
    fn connection_status_line(&self) -> El {
        ...
        // Source is the damascene-ui crate's own `CARGO_PKG_VERSION` (the
    ...
    fn sidebar_inline_empty(
        &self,
        icon_src: damascene_core::SvgIcon,
        headline: &str,
        body: &str,
    ) -> El {
        column([
    ...
    /// One thread row in the sidebar. Uses damascene's `item` widget
    ...
    fn render_inspector_panel(
        ...
        Some(
            scroll([body])
                ...
                .fill(tokens::MUTED.with_alpha_u8(40))
    ...
    /// Build a wrapping-style row of preset chips. Damascene's `row`
    ...
    fn pod_editor_picker_menu(&self, which: PodEditorPicker) -> El {
        ...
        match which {
            ...
            PodEditorPicker::DefaultsBackend => {
                ...
                // labels carry no special styling here — damascene's
    ...
    fn handle_buckets_create_event(&mut self, event: &UiEvent) -> bool {
        ...
        if let Some(modal) = self.buckets_modal.as_mut()
            && let Some(cf) = modal.creating.as_mut()
            && cf.pending_correlation.is_none()
            && damascene_core::widgets::toggle::apply_event_single(
                &mut cf.source_kind,
                event,
        ...
        if let Some(modal) = self.buckets_modal.as_mut()
            ...
            && cf.pending_correlation.is_none()
        {
            if damascene_core::widgets::toggle::apply_event_pressed(
    ...
    fn handle_shared_mcp_editor_event(&mut self, event: &UiEvent) -> bool {
        ...
        if let Some(modal) = self.settings_modal.as_mut()
            ...
            && sub.pending_correlation.is_none()
            && !sub.oauth_in_flight
            && let Some(damascene_core::widgets::toggle::ToggleAction::Selected(raw)) =
                damascene_core::widgets::toggle::classify_event(
                    event,
    ...
    /// damascene tooltip (capped at `STRING_TOOLTIP_BYTES` since damascene's
...
fn empty_state(icon_src: damascene_core::SvgIcon, headline: &str, body: &str) -> El {
...
impl ChatApp {
    ...
    /// (`damascene-core/README.md` "Conversation / event-log row"), we
    ...
    fn event_log_row(
        ...
        match item {
            ...
            DisplayItem::Assistant { text: t } => {
                ...
                let body = damascene_markdown::md_with_options(
                    t,
            ...
            DisplayItem::Image { role, state } => {
                ...
                if *role == ImageRole::User {
                    log_row(gutter, Some(tokens::INFO.with_alpha_u8(64)), body_keyed)
                } else {
...
/// through damascene's [`image()`] widget at a capped display height
...
const DIFF_DEL_FG: Color = Color::srgb_token("diff-del-foreground", 230, 130, 130, 255);
const DIFF_ADD_FG: Color = Color::srgb_token("diff-add-foreground", 150, 220, 170, 255);
...
fn selectable_code_block(key: impl Into<String>, value: &str) -> El {
    // Temporary UI-side guardrail for damascene's soft-wrap height bug:
...
/// upstream `damascene-core/README.md` "Conversation / event-log row"
/// recipe.
