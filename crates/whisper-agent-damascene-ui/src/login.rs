//! [`LoginApp`] — the damascene [`App`] shown before the chat client has
//! credentials.
//!
...
/// gate writing the token to disk; the damascene binary mirrors that).
#[derive(Clone, Debug)]
...
/// The login form as an damascene [`App`]. Owns its own buffers and
