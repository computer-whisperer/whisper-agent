...
//! Mirrors the shape of the other in-tree MCP daemons so the scheduler can talk
//! to all of them with the same client.
...
use crate::tools::{self, ImageGenConfig};

#[derive(Clone)]
pub struct AppState {
    pub cfg: Arc<ImageGenConfig>,
}
...
fn initialize(id: Value, params: Option<Value>) -> JsonRpcResponse {
    ...
    let result = InitializeResult {
        ...
        instructions: Some(
            "Generate images via the OpenAI Images API. Returns one or more PNG \
             images per call as base64-encoded image content blocks."
                .into(),
        ),
    };
