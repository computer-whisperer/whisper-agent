...
| WP1 | WebUI scope selector — `+New bucket` form has a `scope` ComboBox listing `(server)` plus each known pod; pod_id is plumbed through `BucketsEvent` to `ClientToServer::CreateBucket`. Drive-by: every bucket-row action (Build / Pause / Delete / Resync now / Poll now / Search) now reads `pod_id` from the row's `BucketSummary` so clicks on pod-scope rows route to the correct sub-registry instead of the prior hardcoded `pod_id: None`. | `4beb4c4` |
| WP2 | WebUI embedder dropdown — new `ListEmbeddingProviders` wire op + `EmbeddingProvidersList` response surface configured `[embedding_providers.*]` entries (name + kind + endpoint + auth_mode). WebUI fetches once on connection ready (same dedup as `backends_requested`) and replaces the form's freeform `embedder` text input with a ComboBox showing `name (kind)`. Falls back to TextEdit when the catalog is empty so a freshly-spun-up server with no providers still surfaces a typeable field. | `c362975` |
| WP3 | WebUI tracked source kind — adds `BucketSourceInput::Tracked { driver, delta_cadence, resync_cadence }` to the wire with `TrackedDriverInput::Wikipedia { language, mirror }` and `TrackedCadenceInput` (Daily/Weekly/Monthly/Quarterly/Manual). Server's `synthesize_bucket_toml` renders the `[source] kind = "tracked"` block; round-trip tests cover both mirror-set and mirror-omitted cases through `BucketConfig::from_toml_str` to catch wire-vs-TOML rename drift. WebUI form gains a fourth `tracked` source-kind option with driver / language / mirror / delta_cadence / resync_cadence ComboBoxes + text inputs. Defaults to wikipedia + Daily delta + Monthly resync. 831 lib tests passing. | `3b54a5f` |
| WP4 | WebUI per-row state collision fix — `BucketsModalState.{build_progress, build_errors}` re-keyed from `HashMap<String, _>` to `HashMap<(Option<String>, String), _>` so a pod-scope build's progress events don't land in a same-named server-scope row's panel. Drive-by: `BucketDeleted`'s buckets-list `retain` was matching by id alone too — would have dropped the wrong row when a same-named bucket existed in another scope. | `083489e` |
...
| 10+ | Per-pod buckets — ~~lifecycle ops (`CreateBucket`/etc. with `pod_id`), WebUI scope toggle, scoped wire IDs on `QueryBuckets`~~ landed in `PB3a..PB3d` + `WP1..WP4`. Whole feature now in. |
...
- Wire types: `crates/whisper-agent-protocol/src/lib.rs` (search
  `BucketSummary`)
- WebUI modal: `crates/whisper-agent-damascene-ui/src/app.rs` (search `render_buckets_modal`)
