...
[damascene](https://github.com/computer-whisperer/damascene), a small declarative GPU
UI library shaped around how an LLM authors UI. This doc tracks the why, the
crate layout, the architectural choices that fall out of damascene's `App` model,
and where each stage of the port landed.

**Current entry points:**
- Browser: served by the agent itself at `http://$LISTEN_SERVER/` (wasm bundle baked into the release binary; build via `wasm-pack build crates/whisper-agent-damascene-ui --target web`).
- Native: `cargo build -p whisper-agent-desktop-damascene && ./target/.../whisper-agent-desktop-damascene --server http://127.0.0.1:8080`
- Dev all-in-one: `./scripts/dev.sh` (server + daemons; browser at `http://127.0.0.1:8080/`) or `./scripts/dev_damascene.sh` (adds the native client on top).
- Bundle dump: `cargo run -p whisper-agent-damascene-ui --example dump_bundles`

The old egui stack (`whisper-agent-webui`, `whisper-agent-desktop`) has been
removed; damascene is now the only browser + native UI implementation.

## Why damascene

The thesis is the load-bearing argument: damascene is shaped around how a *model*
...
- **Vocabulary parity with shadcn / Radix / Tailwind.** Stock widgets
  ...
  egui code produces *better* damascene code.
- **Build/on_event split is enforced.** `App::build(&self, &BuildCx) -> El`
  ...
  refactor liability as the surface area grows. Damascene structurally prevents
  it.
...
- **One implementation across native + browser.** `damascene-winit-wgpu` for
  desktop, `damascene-web` (wasm + WebGPU) for the browser. Single `App` impl
...
```
crates/
  whisper-agent-damascene-ui/         ← platform-agnostic ChatApp (cdylib + rlib)
    src/
      lib.rs                        re-exports + wasm entry registration
      app.rs                        ChatApp impl of damascene_core::App
  ...
  whisper-agent-desktop-damascene/    ← native binary on damascene-winit-wgpu
...
```rust
impl damascene_core::App for ChatApp {
    ...
    fn theme(&self) -> Theme { Theme::damascene_dark() }
}
```
...
Damascene's `damascene-winit-wgpu` host doesn't currently expose a cross-thread
...
- upstream a wakeup channel in `damascene-winit-wgpu`, or
...
Successive refactors moved the UI off hand-rolled chrome onto damascene's
...
- **Sidebar.** `sidebar([...])` for the surface, `sidebar_header([...])`
  ...
  rows — damascene's "object/action list row" widget gets us the right
...
- **Event-log rows for the chat log, not cards per message.** Per
  upstream damascene guidance (`damascene-core/README.md` "Conversation /
...
- **Markdown for assistant text.** Assistant content goes through
  `damascene_markdown::md_with_options(text, MarkdownOptions::default().math(true))`,
  which walks pulldown-cmark's event API into `paragraph` /
  `bullet_list` / `code_block` / `inlines` / `text(...).code()` etc.
  — same surface a hand-authored damascene tree would use, with native
...
correct without forcing data into the helper mold. Per damascene's
`widget_kit.md`.
...
behaviors). The damascene sidebar is being redesigned around an active-pod
...
- **✅ Slice γ — danger affordances.** Two-click arm-confirm
  ...
  Chrome leans on icons over text per `feedback_damascene_chrome_icons`
  — Run is `zap` (lightning, distinct from Pause's `play`-on-
  resume), Pause is `pause`/`play`, Edit is `square-pen`, Delete
  is `trash`. None of these ship in damascene's built-in
  [`IconName`] registry yet; the lucide-shaped SVGs are bundled
  in `crates/whisper-agent-damascene-ui/src/icons.rs` and constructed
  via `SvgIcon::parse_current_color`. When a future damascene
  ...
  damascene sidebar's `{kind} · {status}` description already
...
modal path; conversation text is now wired into Damascene selection
...
- streaming-args placeholder text (`writing args · {N} chars`) —
  egui shows progress while args JSON is being typed; damascene pushes
...
decoded pixels become an `damascene_core::image::Image` cached inside
`DisplayItem::Image` so rebuilds don't redecode. `event_log_row` then
renders the image through damascene's `image()` widget at a capped
...
Input/staging landed once damascene upstream shipped
`UiEventKind::FileDropped` / `FileHovered` /
`FileHoverCancelled` and `damascene-winit-wgpu` wired the matching
...
- `compose_attachments: Vec<StagedAttachment>` — staged image
  ...
  pre-decoded `damascene_core::Image` for the thumbnail, and a
...
the standard Damascene path: native `damascene-winit-wgpu` turns
...
hosts share `damascene_core::clipboard::{selected_text_for_app,
...
The damascene v1 ships the 80% case as a single scrollable form:
...
plus a preview form item. Damascene doesn't flex-wrap rows, so
...
the first slice that uses damascene's new `BuildCx::is_hovering_within(key)`
...
damascene's `toggle_group_multi`, since the latter doesn't wrap and
...
text input, `max_tokens` / `max_turns` damascene `numeric_input`s,
...
because damascene's `numeric_input` owns its visible text as an
...
headroom for damascene's larger form_item gaps), centered via
...
The radio routes use `damascene_core::widgets::radio::apply_event`
...
`whisper_agent_damascene_ui::LoginApp` is a separate damascene [`App`] —
...
`damascene-markdown` landed upstream and we now route assistant text
through `damascene_markdown::md_with_options(text, MarkdownOptions::default().math(true))`.
...
damascene tree. Got picked up as part of the event-log refactor since
both touched the same code.

Future polish: per `damascene-markdown/src/lib.rs` at the version we
...
lift of `damascene-web/src/lib.rs` with the gfx slot constructor-injected
so the WebSocket / drag-drop / paste JS callbacks can call
`request_redraw` on the same window after pushing inbound events.
The HTML harness in `crates/whisper-agent-damascene-ui/assets/`
...
`crates/whisper-agent-damascene-ui/{pkg,assets}/`. No feature flag — the
cutover was full. `scripts/dev.sh` and `Dockerfile` wasm-pack the
damascene-ui crate before building the agent so the embed sees fresh
output.

Eventual cleanup: upstream a public `damascene-web::start_with::<A>` so
...
- ✅ **JSON tree viewer.** `open_json_viewer(pod_id, path)` mints a
  ...
  `STRING_TOOLTIP_BYTES = 600` since damascene's v1 tooltip is
...
- ✅ **Server settings.** Four operational tabs have landed in the
  Damascene modal: LLM backends (catalog + per-`chatgpt_subscription`
...
- ✅ **Knowledge buckets.** All three phases landed: read-only catalog
  ...
  **Phase 3 detail (search-and-query):** the modal grew a search
  ...
  next to the existing settings cog (bundled SVG — damascene's
...
- ✅ **Thread inspector.** Inline detail panel above the chat log,
  ...
  the state badge). Damascene doesn't yet have an anchored-popover
...
Picked in this codebase, not enforced by damascene:
...
- **Live reload during development.** Damascene doesn't have hot-reload;
...
- **Picking up upstream damascene changes.** Damascene's API is moving while
...
- ~~**`winit` drag/drop events on `damascene-winit-wgpu`.**~~ Landed
  ...
  Clipboard-image paste remains app-side — damascene's
  `text_input.rs` doc explicitly notes "no new damascene API is
...
- `crates/whisper-agent-damascene-ui/src/app.rs` — the App impl + wire
  dispatch + rendering
- `crates/whisper-agent-damascene-ui/examples/dump_bundles.rs` — scene
  catalogue and seeding helpers
- `crates/whisper-agent-desktop-damascene/src/main.rs` — winit host +
  tokio bridge
- `scripts/dev_damascene.sh` — server + daemons + native binary, one
  invocation
- `~/workspace/damascene/damascene.main/docs/LIBRARY_VISION.md` — damascene's
  app/widget-layer architecture (read alongside this doc)
- `~/workspace/damascene/damascene.main/crates/damascene-core/src/widget_kit.md` —
