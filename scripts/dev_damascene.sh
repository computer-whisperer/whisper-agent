#!/usr/bin/env bash
#
# Dev convenience for the damascene native client (whisper-agent-desktop-damascene).
#
...
# desktop-damascene binary in the foreground. Closing the desktop window
...
# Default to skipping the wasm build — the damascene native binary doesn't
...
# Build the desktop-damascene binary up front. dev.sh blocks once the
# server starts; we don't want to be still compiling our binary at
# that point.
echo "==> building whisper-agent-desktop-damascene (release)"
cargo build --release -p whisper-agent-desktop-damascene
...
echo "==> launching whisper-agent-desktop-damascene against http://$LISTEN_SERVER"
...
# login flow itself.
"$REPO_ROOT/target/release/whisper-agent-desktop-damascene" \
